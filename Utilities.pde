                                 ...
                                 float x3, float y3, float x4, float y4)
{
  ...
  int getRectCircleSideCollided(float x1, float y1, float x2, float y2, float x, float y, int r)
  {
    ...
    else if( pointLineDistance( x1, y1, x1, y2, x, y) <= r )   return 1;            // hit the left side of the rectangle
    else if ( pointLineDistance( x1, y1, x2, y1, x, y) <= r )  return 2;            // hit the top side of the rectangle
    else if ( pointLineDistance( x2, y1, x2, y2, x, y) <= r )  return 3;            // hit the right side of the rectangle
    else if ( pointLineDistance( x1, y2, x2, y2, x, y) <= r )  return 4;            // hit the bottom side of the rectangle
